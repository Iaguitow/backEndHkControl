...
function getChartTotalRequests(req, res, next) {
    new Promise((resolve,reject)=>{
        try {
            ...
            sql+="     SUBSTRING(DayOfWeeks(MAX(pr.dtrequested)),1,3) AS daysofweek, ";
...
function getChartReqToBeDone(req, res, next) {
    new Promise((resolve,reject)=>{
        try {
            ...
            var sql = "";
            sql+=" SELECT * FROM (SELECT ";
                sql+="     MAX(pr.people_has_requests) AS people_has_requests, ";
                sql+="     DayOfWeeks(MAX(pr.dtrequested)) AS dayweek, ";
                sql+="     MAX(pr.dtrequested) AS dtrequested, ";
                sql+="     MAX(pr.dtrequestdone) AS dtrequestdone, ";
                sql+="     MAX(rt.resquestdescription) AS resquestdescription, ";
